...
must produce the exact same trajectory as the Python reference
implementation, step for step -- otherwise it isn't a valid drop-in and
must not be wired up as an alternative backend.
...
def _config_tuple(config: PhysicsConfig) -> tuple[float, float, float, float, float]:
    return (
        config.gravity,
        config.friction,
...
def _run_native(
    spheres: list[tuple[float, float, float, float, float, float, float, int]], steps: int
) -> list[tuple[float, float, float, float, float, float, float, int]]:
    for _ in range(steps):
...
def _isclose(a: Vector3, b: Vector3) -> bool:
    return all(
...
def _assert_matches(python_result: list[Sphere], native_result: list[tuple]) -> None:
    ...
    for sphere, (x, y, z, vx, vy, vz, radius, level) in zip(
        python_result, native_result, strict=True
    ):
        assert _isclose(sphere.position, Vector3(x, y, z))
        assert _isclose(sphere.velocity, Vector3(vx, vy, vz))
