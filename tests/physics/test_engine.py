...
def test_friction_slows_sphere_resting_on_floor() -> None:
    ...
    config = PhysicsConfig(gravity=0.0, friction_min=0.2, friction_max=0.2)
    step([s], dt=0.01, boundary=FIELD, config=config)
    assert s.velocity.x == pytest.approx(2.4)


def test_friction_slows_fast_sphere_less_than_slow_sphere() -> None:
    fast = Sphere(Vector3(0.0, 0.0, 0.5), Vector3(10.0, 0.0, 0.0), radius=0.5, level=0)
    slow = Sphere(Vector3(0.0, 0.0, 0.5), Vector3(0.5, 0.0, 0.0), radius=0.5, level=0)
    config = PhysicsConfig(
        gravity=0.0, friction_min=0.01, friction_max=0.3, friction_speed_threshold=6.0
    )
    step([fast], dt=0.01, boundary=FIELD, config=config)
    step([slow], dt=0.01, boundary=FIELD, config=config)

    fast_fraction_lost = 1 - fast.velocity.x / 10.0
    slow_fraction_lost = 1 - slow.velocity.x / 0.5
...
def test_collision_filter_excludes_matching_pairs_from_resolution() -> None:
    ...
    step(
        ...
        dt=0.01,
        boundary=FIELD,
        config=PhysicsConfig(gravity=0.0, friction_min=0.0, friction_max=0.0),
