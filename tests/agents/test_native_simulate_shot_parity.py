...
`test_determinism.py`'s docstring) into a still-tiny but no-longer-
single-bit difference by the end.
...
SPAWN = Vector2(FIELD.x_min + 1.0, FIELD.y_min + 1.0)
...
def _isclose_vec(a: Vector2, b: Vector2) -> bool:
    return all(
